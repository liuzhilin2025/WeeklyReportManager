...
import com.practice.weeklyreportmanager.service.tool.WeeklyReportChangedEvent;
...
public class WeeklyReportServiceImpl implements WeeklyReportService {
    ...
    public WeeklyReport saveWeeklyReport(WeeklyReportDTO dto, Long userId) {
        ...
        // 发布在方法体内（事务内），监听器挂在 AFTER_COMMIT 上，提交成功后才会真正索引
        eventPublisher.publishEvent(new WeeklyReportChangedEvent(report));
    ...
    public WeeklyReport updateWeeklyReport(WeeklyReportDTO dto, Long id, Long userId) {
        ...
        // 发布在方法体内（事务内），监听器挂在 AFTER_COMMIT 上，提交成功后才会真正索引
        eventPublisher.publishEvent(new WeeklyReportChangedEvent(existing));
        return existing;
    }
    ...
    @Override
    public List<WeeklyReport> getAllSubmittedReports() {
        QueryWrapper<WeeklyReport> wrapper = new QueryWrapper<>();
        wrapper.eq("status", "SUBMITTED");
        wrapper.orderByAsc("week_start_date");
        return weeklyReportMapper.selectList(wrapper);
