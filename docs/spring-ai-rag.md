...
| **只看某段时间的卡** | metadata 的 `weekStartDate` 过滤 | ⚠️ **这个功能在内存实现上失效了**，见第六节坑 2 |
...
三个参数各有一个不容做错的理由，见第五节。

### 3.2 存储：`VectorStore` Bean
...
见 3.3。这是被实现逼出来的混合策略——**不要赌向量库的过滤能力**。
...
**修复**：日期范围改到应用层筛（见 3.3）。多召回（`topK=20`）再筛选（`limit=5`），给筛除留余量。
